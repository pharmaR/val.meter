...
#' @export
pkg_data_get_scopes <- new_generic("pkg_data_get_tags", c("field"))
...
#' @export
pkg_data_get_suggests <- new_generic("pkg_data_tags", c("field"))

method(pkg_data_get_suggests, class_character) <-
  function(field, suggests, ...) pkg_data_get_suggests(as_pkg_data(field))

method(pkg_data_get_suggests, S7::new_S3_class(pkg_data_s3_class())) <-
  function(field, suggests, ...) ""  # default, no suggests
...
method(is, list(class_any, class_any)) <-
  function(x, class) FALSE

method(is, list(class_any, class_character)) <-
  function(x, class) inherits(x, class)
...
method(is_subclass, list(new_S3_class("S7_base_class"), class_any)) <-
