...
Local source code is distinct from other local sources because it
carries the assumption that all files needed to fully reproduce the
original package are bundled alongside.
...
Unlike package archives that might be distributed or local installs,
source code contains all tests and files which may be ignored as part of
the build process, yet may be informative for metric assessment.
