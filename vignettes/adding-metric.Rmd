...
vulnerabilities (OSV) database. In our case, we can use a known example to see
how vulnerabilities might be reported:

```{r, eval = requireNamespace("rosv", quietly = TRUE)}
rosv::osv_query(
  "haven",
  version = "0.1.1",
  ecosystem = "CRAN",
...
reported. But we can also see a fair amount of information provided by `rosv`.
...
output of `rosv` as intermediate data, which we'll then use to derive our
metrics as summarizing data.

What's important to note is that `rosv` operates on packages from `CRAN`, so
...
And similarly, if we didn't have `rosv` installed we'd see something like
...
testthat::with_mocked_bindings(
  ...
  requireNamespace = function(package, ...) {
    if (package == "rosv") FALSE
    else orig_require_namespace(package, ...)
  },
  {
    r <- cran_repo_resource(
      "haven",
      "0.1.1",
...
Now that we can fetch the `rosv` response, we can process it to derive our
metric. 
...
`rosv_vulnerability_df`, which itself is limited to only CRAN repositories. We
